...
def query_gpt_completion(few_shot, prompt_ls, uuids, ground_truth_answers, pred_dir, pred_file_name, max_prompt_size=500, temperature=0, max_tokens=100):
  few_shot_prompt_ls = ["\n".join([f"{c} {a}" for c, a in shots]) + f"\n{prompt_ls[i]}" for (i, shots) in enumerate(chunks(list(zip(few_shot[1], few_shot[3])), 5)) if i < len(prompt_ls)]
  ...
  prompt_size = max([len(prompt.split()) for prompt in few_shot_prompt_ls])
  assert prompt_size < max_prompt_size

  response = openai.Completion.create(
      model="text-davinci-002",
      prompt=few_shot_prompt_ls,
  ...
  y_pred_strings = [response["choices"][i]["text"].strip().lower() for i in range(len(few_shot_prompt_ls))]
  y_preds = [y_pred_string.split() for y_pred_string in y_pred_strings]
  y_preds = [y_pred[0] if len(y_pred) > 0 else '' for y_pred in y_preds]

  response_pairs = [t for t in zip(uuids, response)]
  response_string_tpls = [t for t in zip(uuids, y_pred_strings, ground_truth_answers)]
  result_pairs = [t for t in zip(uuids, y_preds)]
...
class CLUTRRDataset:
  ...
  def __getitem__(self, i):
    ...
    context = context
    task = datapoint[10]
    query = f" So {query_sub_obj[1]} is {query_sub_obj[0]}'s:"
    answer = datapoint[5]
    prompt = context + query
    return ((task, uuid), prompt, query, answer)
...
def clutrr_loader(root, dataset, batch_size, k):
  train_dataset = CLUTRRDataset(root, dataset, "train", k)
  train_loader = DataLoader(train_dataset, 80, collate_fn=CLUTRRDataset.collate_fn, shuffle=True)
  ...
  def test_epoch(self, epoch):
    ...
    with torch.no_grad():
      ...
      for (i, (uuids, xs, queries, ys)) in enumerate(iterator):
        few_shot = next(self.train_loader)
        y_preds = query_gpt_completion(few_shot, xs, uuids, ys, self.pred_dir,  f"{self.pred_file_name}_{i}")
        for uuid, y_pred in zip (uuids, y_preds):
          all_y_pred[uuid] = y_pred

        (num_correct, batch_size) = self.accuracy(y_preds, ys)
        total_count += batch_size
        total_correct += num_correct
        correct_perc = 100. * total_correct / total_count

        iterator.set_description(f"[Test Epoch {epoch}], Accuracy: {total_correct}/{total_count} ({correct_perc:.2f}%)")
...
if __name__ == "__main__":
  ...
  parser.add_argument("--pred_dir", type=str, default="gpt_preds_5_shot_pred_all")
  ...
  # Load the dataset
  train_loader, test_loader = clutrr_loader(data_root, args.dataset, args.batch_size, args.k)

  # Train
  trainer = Trainer(train_loader, test_loader, device, args.pred_dir, args.pred_file_name, args.learning_rate, num_mlp_layers=args.num_mlp_layers, debug=args.debug, no_fine_tune_roberta=args.no_fine_tune_roberta)
