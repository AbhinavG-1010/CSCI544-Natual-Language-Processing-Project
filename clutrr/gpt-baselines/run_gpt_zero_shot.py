...
def query_gpt_completion(prompt_ls, uuids, ground_truth_answers, pred_dir, pred_file_name, max_prompt_size=300, temperature=0, max_tokens=100):
    ...
    y_pred_strings = [response["choices"][i]["text"].strip().lower() for i in range(len(prompt_ls))]
  ...
  def test_epoch(self, epoch):
    ...
    with torch.no_grad():
      ...
      for (i, (uuids, xs, queries, ys)) in enumerate(iterator):
        y_preds = query_gpt_completion(xs, uuids, ys, self.pred_dir,  f"{self.pred_file_name}_{i}")
...
if __name__ == "__main__":
  ...
  parser.add_argument("--pred_dir", type=str, default="gpt_preds_zero_pred_all")
